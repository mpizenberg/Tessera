...
| `packages/client`             | `cardano-tessera-client`: the typed client over a Tessera backend's HTTP contract — payload types and constants, one method per route decoding into `cip-179` types, the contract version check, and the per-network epoch calendar.                    |
| `packages/core`               | Tessera app core (`cardano-tessera-core`): the `DataSource` seam, in-memory keyset paging, the survey-list aggregation adapter, and the read configuration. The HTTP contract lives in `cardano-tessera-client`, the domain/tally surface in `cip-179`. |
...
| `packages/amaru-store-reader` | The verifier's inputs read from an Amaru node's own stores, a Rust crate outside the pnpm workspace; the recipe is in its README.                                                                                                                       |
| `packages/amaru`              | The verifier's inputs from what `amaru-store-reader` printed, instead of Koios.                                                                                                                                                                         |
