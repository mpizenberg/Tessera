...
| ----------------------------------- | ----------------------------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Stakeholder stake (the mark)        | `POST /account_stake_history?epoch_no=eq.E+2`                                 | **bulk** (many stake addresses) | the stake snapshot taken at the end of `E`, which db-sync labels `E+2`; exact and historical, readable once the readiness check below passes; one row per **pool-delegated** account — a registered account with no row counts with weight 0. (The similarly-named `/account_history` is the deprecated variant — don't use.)                                                                                                                                                                                                                                                                    |
| Stakeholder membership at `E`'s end | `POST /account_update_history?epoch_no=lte.E`                                 | **bulk**                        | registration/deregistration event rows; registered at `E`'s end iff the last event in `absolute_slot` order isn't a deregistration. Read **newest-first and stopped** at the first page that settles an address, the rest re-asked as a narrower batch — only an account's newest slot decides, so a lifetime of registration churn costs no more than a single registration and never crowds out the batch it shares. (`/account_updates` is the deprecated variant.)                                                                                                                           |
| DRep voting power (distribution)    | `GET /drep_voting_power_history?_epoch_no=E+1&epoch_no=eq.E+1&drep_id=in.(…)` | **bulk** (many DRep ids)        | the distribution taken at the end of `E`, which db-sync labels `E+1`; exact; a row per DRep **some account has delegated to** then, and only for a DRep registered then, so a row proves registration. An absent id is either unregistered or registered with nothing delegated, which the membership read below separates. Chunked at 50 ids (~3 KB URL), verified live to return amounts byte-identical to the single-`_drep_id` form. Both epoch filters: `_epoch_no` bounds the server-side query, and the PostgREST column filter guards against its known misbehaviour for current epochs. |
| DRep membership at `E`'s end        | `GET /drep_updates?drep_id=in.(…)&block_time=lte.<end of E>`                  | **bulk** (many DRep ids)        | registration events, read **only** for the ids the power query returned nothing for — so a survey whose DRep responders all have delegators pays nothing for it. Registered at `E`'s end iff the newest event at or before it is not a deregistration (`updated` filtered out server-side: it leaves registration unchanged). No epoch column, so the boundary is `/epoch_info`'s `end_time`, and an epoch whose boundary or whose within-block order can't be read (a registration and a deregistration sharing one block) postpones rather than be guessed.                                    |
| Readiness                           | `GET /tip`, `GET /pool_list?…&limit=1`, `GET /pool_stake_snapshot`            | per epoch                       | nothing above is read before 12 hours past `E+1`'s start, and before Koios's stake cache holds the mark (`pool_stake_snapshot` serves a row with `epoch_no = E+2` through any active pool; the cache is filled only once db-sync has written the whole snapshot, which it does in slices over the first ~700 blocks of `E+1` on mainnet) or the tip is at `E+3` or later. The margin covers the Koios instances behind one URL, since the cache proves only the one that answered: a partial mark reads as weight 0.                                                                             |
...
| Stakeholder total                   | `GET /epoch_info` (`E+2`) or `GET /pool_stake_snapshot`                       | per epoch                       | the mark's total active stake: from `/epoch_info` once `E+2` has begun, from the stake cache's row before. **Flaky**: some (preview) epochs answer db-sync `word128` errors — treated as "unavailable, retry next cron", never fatal.                                                                                                                                                                                                                                                                                                                                                            |
| DRep total                          | `GET /drep_epoch_summary` (`E+1`)                                             | per epoch                       | total DRep voting power of the same distribution.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **Finalization** (implemented in `backend/server/src/finalize.ts`, run at the
  ...
  Weights and totals wait for Koios's readiness check too (§6.1): the mark is
  written over the first blocks of `E+1`, and the pass waits 12 hours past
  `E+1`'s start, so a survey's artifact comes about that long after its end
  epoch closes.
...
fetches the artifact from a backend, and its bundle only to name differences,
refetches every verification input straight from Koios, or from two Dolos
nodes replayed from Mithril-certified immutable files (`packages/dolos`; how
to run an audit and what each source trusts: `docs/AUDIT.md`), re-runs the pinned ruleset through the same shared code, and compares content
