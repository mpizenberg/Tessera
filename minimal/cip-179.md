...
- Public or timelocked ballots, the latter using Drand `tlock` for delayed reveal.
...
;   ballot is padded to before encryption, so ciphertext length does not leak
;   answer content.
timelocked_ballot_mode = [1, drand_chain_hash, pos_uint, pos_uint]
                          ;   chain_hash        round     padding_size

ballot_mode = public_ballot_mode / timelocked_ballot_mode
...
survey_definition = [
  ...
  ballot_mode,                 ; how responses are submitted
...
; Response answers depend on the referenced survey's ballot_mode:
...
```
[specVersion, owner, title, description, roleWeighting, endEpoch, ballotMode, questions]
```
...
| 6 | ballot_mode | How responses are submitted: `[0]` public, or `[1, ...]` timelocked. |
...
Version 3 extends version 2 with the following breaking changes. A `ballot_mode` field is added to `survey_definition` as a tagged sum type (`[0]` public, `[1, chain_hash, round, padding_size]` timelocked), enabling timelocked ballots: responses are submitted as Drand `tlock` ciphertext and become decryptable by anyone once the pinned round publishes (delayed reveal, not permanent secrecy). The `survey_definition` array is reordered so the fixed-size header fields precede the variable-length `questions` array, which now appears last: `[specVersion, owner, title, description, roleWeighting, endEpoch, ballotMode, questions]`. Question indices are unchanged, so `survey_response` answer references are unaffected. The `survey_response` answers field accepts either plaintext answer items (public surveys) or a chunked-bytes tlock ciphertext (timelocked surveys). A `specVersion` field is restored as the first element of `survey_response` (it was present in version 1, then dropped in version 2 on the assumption the referenced survey supplied it); without it, a response — including whether its answers are plaintext or ciphertext — cannot be decoded without first resolving the survey definition.
