...
- Increment 3 complete in portable source: preprod has a distinct Worker/D1 environment, generated operator-local resource configuration, migration/deploy scripts, a health-gated creation and rollback runbook, and a strict three-network Cloudflare/D1/Tessera metrics collector replacing the Preview-only baseline script. Full checks and Preview/preprod Worker dry runs pass. Resource creation, migration, first-snapshot validation, and live collection remain operator state because Wrangler is not authenticated in this environment.
- Increment 3 follow-up: local development is now per-network too — `dev:preview`/`dev:preprod`/`dev:mainnet` on the backend and `dev:preprod` on the app, with the Node cache file defaulting to `./tessera-cache-$NETWORK.sqlite` so two networks cannot reconcile each other's rows out of one file. Verified live: `dev:preprod` reports `{"ok":true,"network":"preprod"}` against `preprod.koios.rest`. Full checks pass.
...
- **Derive the local SQLite cache path from the network instead of pinning it per dev script.** The alternative was to keep the single `./tessera-cache.sqlite` default and set `DB_PATH` inside each `dev:<network>` script. The cache carries no network column, so a shared file lets each refresh delete the other network's rows as absent from its authoritative scan; putting the guard in the scripts would leave a bare `NETWORK=preprod pnpm dev` broken. Deriving the default makes the collision unrepresentable. Cheaply reversible, but it renames the existing Preview cache file, so a running local backend re-scans once.
- **Give the app only `dev:preprod`, not a symmetric `dev:preview`.** Vite mode `preview` is a deploy mode: `.env.preview` pins `VITE_INDEXER_URL` to the deployed Worker and mode files outrank `.env.local`, so a `dev:preview` script would silently bypass the local backend. Mode `preprod` leaves that variable unset today and resolves to `.env.local`. If `.env.preprod` later pins a deployed URL, `dev:preprod` inherits the same trap and will need an explicit local override.
...
- **Generate preprod's Wrangler config locally from a committed placeholder.** The alternatives were to commit the operator's D1 UUID or require an untracked edit to the canonical config. A mode-0600, git-ignored generated config keeps account resource identity out of portable source and lets migration, secret, deploy, and rollback commands share one exact configuration. Cheaply reversible; the generator can be removed if the resource identity later becomes project-owned configuration.
- **Replace the Preview baseline collector with one strict network-parameterized collector.** The alternative was to retain separate baseline and workload scripts, which would duplicate Cloudflare queries and allow their report schemas to drift. The replacement requires an explicit window/workload/cadence, verifies standard resource names plus both health identities, and emits the same report shape for all three networks. Cheaply reversible, though retaining two collectors would reintroduce duplicate concepts.
