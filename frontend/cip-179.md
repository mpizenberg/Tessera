...
Linkage is a **discovery/advertisement** relationship plus an **epoch-alignment** invariant. It does **not** restrict who may respond: every role in `eligible_roles` participates exactly as it would standalone, and casting a governance vote on the linked action is an _optional_ enrichment, not a precondition ([Linked survey response rules](#linked-survey-response-rules)).
