...
| `src/domain`     | App-side domain: transaction building (`create.ts`, `respond.ts`, `fee.ts`), role checks, artifact render-model (`artifactView.ts`). Pure read/tally rules live in `@tessera/core` (thin re-export shims here keep old import paths working). |
| `src/wallet`     | CIP-30 wallet discovery, connection, signing, submission.                                                                                                                                                                                     |
| `src/tlock`      | Sealed mode: drand timelock encryption of answer payloads.                                                                                                                                                                                    |
