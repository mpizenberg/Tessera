...
| 2 | Mechanism-B verdicts pinned to gov links known at first validation, never re-checked | done (pending commit) — persist `linked_action_id` (migration 0004), re-validate bindable rows on link change, defer verdicts when gov-links fetch unreliable |
