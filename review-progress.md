...
| # | Finding (short) | Status |
| --- | --- | --- |
| 5 | Weight-fetch resume cursor only resumes between roles, not within one; snapshot scan has no resume | done 03d9a03 — `fillWeights` now persists per chunk (per-credential DReps, per-50 stakeholders). Scan-resume sub-point done e3411c2: fetch-once `tx_metadata_cache` (migration 0005) banked per fulfilled batch; over-budget scans converge across crons |
...
| 19 | Post-close cancellations invisible outside the artifact (list shows "Ended", raw tally in direct mode) | done c3e8a23 — backend `/api/surveys` now carries `finalizedCancelled` (survey keys whose artifact records a cancellation), derived from the stored artifact JSON via `json_extract` (no schema change/migration needed); `aggregateSurveyList` overlays it so Explore/detail show "cancelled" past close. Direct-Koios mode remains uncovered: client-side proof fetching for closed surveys was considered and REJECTED — the per-scan cost is permanent/cumulative and adversarially inflatable (one tx fee buys extra Koios requests on every future scan); rationale documented in koios.ts's fetchAll |
