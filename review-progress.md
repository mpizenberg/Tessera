...
| 19 | Post-close cancellations invisible outside the artifact (list shows "Ended", raw tally in direct mode) | done c3e8a23 — backend `/api/surveys` now carries `finalizedCancelled` (survey keys whose artifact records a cancellation), derived from the stored artifact JSON via `json_extract` (no schema change/migration needed); `aggregateSurveyList` overlays it so Explore/detail show "cancelled" past close. Direct-Koios mode remains uncovered by design (no artifacts there) — would need the option-B client-side proof fetch |
